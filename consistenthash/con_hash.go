...
func (m *Map) Get(key string) string {
	...
	// 二分查找，传入长度和一个匿名函数，search只关心索引的查找算法，怎么查，去哪里查，由匿名函数决定，这个是闭包closure思想的体现，即逻辑与数据分离
	// 否则针对不同类型数据要有不同的search实现，增加代码复杂度
	...
	// 在RLock中进行了写操作，不符合go的并发规范
	// 在 Go 语言中，普通 Map 的并发写入被设计为直接 Panic（崩溃），而且这种 Panic 是**无法恢复（Unrecoverable）**的 fatal error。
	// to do: 拆分读写锁逻辑
...
func (m *Map) addNode(node string, replicas int) {
	for i := 0; i < replicas; i++ {
		//计算虚拟节点的哈希值，涉及字符串到字节流的转换和字节流到整数的转换，以及哈希计算；
		hash := int(m.config.HashFunc([]byte(fmt.Sprintf("%s-%d", node, i))))

		//若发生扩容或增加删除节点操作，slice的地址或元信息可能会变化，所以要重新赋值。
		m.keys = append(m.keys, hash)
		//int to string 映射, 哈希值到虚拟节点映射
		m.hashMap[hash] = node
	}
	//节点到虚拟节点数量的映射
